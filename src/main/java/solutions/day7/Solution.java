...
public class Solution {
    ...
    public long getSolution2(List<String> lines) {
        List<Hand> hands = new ArrayList<>();
        for (String line : lines) {
            Hand hand = createHandWithJoker(line);
            hands.add(hand);
        }
        List<Hand> sortedHands = sortHandsWithJoker(hands);
    ...
    private Hand createHandWithJoker(String line) {
        String[] split = line.split(" ");

        String cards = split[0];
        Type typeOfHand = getTypeOfHandWithJoker(cards);
        Hand hand = new Hand(cards, Long.parseLong(split[1]), typeOfHand);

        return hand;
    }

    private Type getTypeOfHandWithJoker(String cards) {
        Map<Character, Integer> labelCounts = new HashMap<>();
        for (int i = 0; i < cards.length(); i++) {
            char c = cards.charAt(i);
            labelCounts.merge(c, 1, Integer::sum);
        }
        if (labelCounts.containsKey('J') && labelCounts.size() > 1) {
            Integer jokerCount = labelCounts.get('J');
            labelCounts.remove('J');
            Map.Entry<Character, Integer> entryWithHighestCount = getEntryWithHighestCount(labelCounts);
            labelCounts.put(entryWithHighestCount.getKey(), entryWithHighestCount.getValue() + jokerCount);
        }
        Type type;
        switch (labelCounts.size()) {
            case 1 -> type = Type.FIVE_OF_A_KIND;
    ...
    private Map.Entry<Character, Integer> getEntryWithHighestCount(Map<Character, Integer> labelCounts) {
        Map.Entry<Character, Integer> entry = Collections.max(labelCounts.entrySet(), Map.Entry.comparingByValue());
        return entry;
    }

    private List<Hand> sortHandsWithJoker(List<Hand> hands) {
        List<Hand> sortedHands = hands.stream().sorted(Solution::compareHandsWithJoker).toList();
        return sortedHands;
    }

    private static int compareHandsWithJoker(Hand hand1, Hand hand2) {
        Type type1 = hand1.type;
        Type type2 = hand2.type;
